...
function ServiceProvider(providerProvider) {
			...
			if( services.hasOwnProperty(name) ) {
				...
				.then(function(provider) {
					...
					if(!provider.hasOwnProperty('$get')) {
						return Promise.reject( new Error('Provider '+name +' needs a $get attribute!') );
					} else {
						return services[name] = that.$injector.invoke(provider.$get, {}, null, function(name) {
